...
Standard: replicate ReadyAPI. Same properties-step field, last nonempty
``setPropertyValue`` wins (Groovy execution order). Do not reorder JSON
extracts vs location-header slices vs JDBC vs generators so a "better"
id wins -- that is how B2B-6604 confirm-validation received a guest id.
See ``_last_nonempty_setproperty`` / ``_should_emit_setproperty``.
...
def _emit_def_publications(script: str, bindings: dict, ctx: dict,
    ...
    for m in re.finditer(
        ...
        if not _dest_owned_by_var(script, dest_key, new_var):
            continue
        ...
        out.extend([
            ...
            f'{step_name_hint} -- later calls will 401. Check '
            f'salesforce_assertion / sf_config (do not invent credentials)");',
            f'    }}',
    ...
    for var_name, info in bindings.items():
        ...
        if not _dest_owned_by_var(script, dest_key, var_name):
            continue
    ...
    for var_name, info in bindings.items():
        ...
        # putExtracted (not putIfNonEmpty): ReadyAPI `def hiltonmemberid`
        # is a per-script local. Owner TOTP then member TOTP both bind
        # that name; putIfAbsent left JDBC querying the owner id and
        # confirmValidation_200_2 sent the consumed OTP (400).
        out.append(
            f'    TestSupport.putExtracted(ctx, "{var_name}", {var_name});')
        out.append('}')
        published.add(var_name)
...
def _has_bearer_concat(script: str) -> bool:
    ...
    if not script:
        return False
    stripped = _strip_groovy_comments(script)
...
def _strip_groovy_comments(script: str) -> str:
    """Remove line and block comments so recognizers ignore commented-out code."""
    stripped = re.sub(r'//[^\n]*', ' ', script)
    return re.sub(r'/\*.*?\*/', ' ', stripped, flags=re.DOTALL)


def _has_active_groovy_def(script: str, var_name: str) -> bool:
    """True when `def var_name = ...` exists on a non-comment line."""
    return bool(re.search(
        rf'(?m)^(?!\s*//)\s*def\s+{re.escape(var_name)}\s*=',
        script))
...
def _balanced_arg_call(script: str, call_prefix_rx: str) -> list[tuple[str, ...]]:
    ...
    list body."""
    # Commented `//sql.execute(sql_query)` must not trigger mutation SkipException
    # when sibling active `sql.execute(sql_addressline1)` calls translate fine.
    script = _strip_groovy_comments(script)
    results = []
...
def _last_nonempty_setproperty(script: str) -> dict[tuple[str, str], str]:
    ...
    Blank clears (``setPropertyValue("X", "")``) are skipped so they do
    not hide a later real write. A later nonempty write of the same
    field replaces the earlier one -- matching Groovy execution order.

    Used so JSON extracts and location-header slices do not clobber a
    field the SoapUI author overwrote later (B2B-6604: split('/')[0]
    guest id then ``resp_obj.memberId``).
    """
    last: dict[tuple[str, str], str] = {}
    for step, field, expr in _find_setproperty_targets(script):
        if expr in ('""', "''"):
            continue
        last[(step, field)] = expr
    return last


def _should_emit_setproperty(
        last: dict[tuple[str, str], str],
        target_step: str, field: str, expr: str) -> bool:
    """ReadyAPI last-write-wins gate for one ``setPropertyValue`` emit.

    Blank clears never emit. A nonempty write emits only when it is the
    last nonempty assignment of ``(target_step, field)`` in the Groovy
    script -- the value ReadyAPI would leave in that property.
    """
    if expr in ('""', "''"):
        return False
    return last.get((target_step, field)) == expr


def _bare_setproperty_expr(expr: str) -> str:
    return re.sub(r"(\.toString\(\))?(\.trim\(\))?\s*$", "", expr.strip())


def _dest_owned_by_var(script: str, dest_key: str, var_name: str) -> bool:
    """False when a later setPropertyValue of dest_key is not this var.

    Def-publication must not plant an earlier slice/extract into a field
    ReadyAPI overwrote afterward (guest-id slice then resp_obj.memberId).
    """
    if "." not in dest_key:
        return True
    step, field = dest_key.split(".", 1)
    last_expr = _last_nonempty_setproperty(script).get((step, field))
    if last_expr is None:
        return True
    return _bare_setproperty_expr(last_expr) == var_name


def _split_top_plus(expr: str) -> list[str]:
    """Split ``a + " " + b`` on top-level ``+`` (not inside quotes)."""
    parts: list[str] = []
    buf = ""
    in_str = None
    esc = False
...
def _space_concat_idents(expr: str) -> Optional[list[str]]:
    """ReadyAPI ``generatedUsername2 + " "+generatedUser3`` → word idents.

    Returns None when the expr is not ident joined by space-string literals.
    """
    if not expr or "+" not in expr:
        return None
    parts = [p.strip() for p in _split_top_plus(expr)]
    if len(parts) < 3:
        return None
    idents: list[str] = []
    expect_ident = True
    space_lit = re.compile(r'''^["'] ["']$''')
    for p in parts:
        if expect_ident:
            ...
            if not space_lit.match(p):
                return None
            expect_ident = True
    if expect_ident or len(idents) < 2:
        return None
    return idents


def _groovy_ident_aliases(script: str) -> dict[str, str]:
    """``def generatedUsername2 = generatedUser2`` → alias hops (ReadyAPI)."""
...
def _alias_root(name: str, aliases: dict[str, str]) -> str:
    seen: set[str] = set()
    cur = name
...
def _generator_regexify(script: str, ident: str) -> str:
    """ReadyAPI ``generator(('A'..'Z').join(), 8)`` → faker regexify charset."""
    m = re.search(
        rf"def\s+{re.escape(ident)}\s*=\s*generator\s*\((.*?)\)",
        script, re.S)
    if m and re.search(r"""['\"]A['\"]\s*\.\.\s*['\"]Z['\"]""", m.group(1)):
        return "[A-Z]{8}"
    return "[a-z]{8}"


def _space_concat_overlay_lines(
        script: str,
        last_setproperty: dict[tuple[str, str], str]) -> list[str]:
    """Overwrite generateStandard name/name2 with ReadyAPI space-concat.

    Shared Groovy locals (and ``def a = b`` aliases) share one Java
    random so TA ``companyName`` (name) and create-account ``name``
    (name2) match the way ReadyAPI DataGenInput wrote them.
    """
    concat_pubs: list[tuple[str, str, list[str]]] = []
    aliases = _groovy_ident_aliases(script)
    for target_step, field, expr in _find_setproperty_targets(script):
        if not _should_emit_setproperty(
                last_setproperty, target_step, field, expr):
            continue
        idents = _space_concat_idents(expr)
        if idents:
            concat_pubs.append(
                (target_step, field,
                 [_alias_root(v, aliases) for v in idents]))
    if not concat_pubs:
        return []
    lines = [
        "    // [translated] ReadyAPI space-concat names (shared generator vars)",
    ]
    seen_vars: list[str] = []
    for _s, _f, idents in concat_pubs:
        for v in idents:
            if v not in seen_vars:
                seen_vars.append(v)
    for v in seen_vars:
        pat = _generator_regexify(script, v)
        lines.append(
            "    String __gc_" + v
            + ' = FakeData.faker().regexify("' + pat + '");')
    for step, field, idents in concat_pubs:
        join_expr = ' + " " + '.join("__gc_" + v for v in idents)
        lines.append(
            "    TestSupport.putExtracted(ctx, "
            f'"{_ctx_key(step, field)}", {join_expr});')
    return lines


def _emit_incomplete_guestid_extract(
        script: str, ctx: dict) -> list[str] | None:
    """ReadyAPI XML sometimes drops the final ``setPropertyValue("guestId",
    resp_obj.guestId...)`` line but keeps parseText setup (B2B-4878,
    B2B-3503). Infer publish to ``PropertiesGuestId.guestId`` when the
    script clears guestId, parses an enroll response, and has no nonempty
    guestId write."""
    if "PropertiesGuestId" not in script:
        return None
    if not re.search(
            r'[Jj]son[Ss]lurper\w*\s*(?:\(\))?\s*\.parseText\s*\(',
            script):
        return None
    for target_step, field, expr in _find_setproperty_targets(script):
        if field == "guestId" and expr not in ('""', "''"):
            return None  # complete script -- generic setproperty_extract owns it
    source_matches = _JSON_EXTRACT_RX.findall(script)
    if not source_matches:
        return None
    source_step = source_matches[0]
...
def translate(script: str, response_var_by_step: dict[str, str],
              step_name_hint: str = "",
              suite_cleanup_fqn: str = "") -> tuple[list[str], dict]:
    ...
    # CleanupDB. When this suite emitted SuiteCleanup, call that hook
    # instead of skipping. Other cross-suite runs (Library, smoketest, ...)
    # stay skipped.
    elif ("testSuites" in script
          and re.search(r'\.run\s*\(\s*testRunner', script)):
        is_cleanup_run = bool(re.search(
            r'Cleanup_testdata_creation|cleanup_db|testSteps\s*\[\s*["\']CleanupDB["\']\s*\]',
            script, re.I))
        if is_cleanup_run and suite_cleanup_fqn:
            lines.append(
                f'// testdata-cleanup: ReadyAPI ran CleanupDB here; '
                f'{suite_cleanup_fqn}.afterEachTest(ctx) runs from '
                f'@AfterMethod instead.')
            lines.append(
                f'LOG.info("Imported testdata-cleanup skipped at start of '
                f'test; {suite_cleanup_fqn}.afterEachTest(ctx) runs after '
                f'this method");')
            patterns_matched.append("suite_cleanup_deferred_to_after")
        else:
            lines.append(
                'LOG.warn("cross-suite Groovy call skipped (suite not imported): '
                f'{step_name_hint}");')
            patterns_matched.append("cross_suite_run_skip")
        consumed = True

    # ---- generateRandomAccountID -> Properties.AccountID
    elif re.search(r'def\s+generateRandomAccountID\s*\(', script):
        field = "AccountID"
        mset = re.search(
            r'setPropertyValue\s*\(\s*["\']([^"\']+)["\']', script)
        if mset:
            field = mset.group(1)
        lines.extend([
            '// [translated] generateRandomAccountID',
            '{',
            '    String accountID = FakeData.faker().regexify("[A-Z0-9]{9}");',
            f'    TestSupport.putExtracted(ctx, "Properties.{field}", accountID);',
            '}',
        ])
        patterns_matched.append("generate_random_account_id")
        consumed = True

    # ---- generateRandomEmail
    elif re.search(r'def\s+generateRandomEmail\s*\(', script):
        field = "Email"
        mset = re.search(
            r'setPropertyValue\s*\(\s*["\']([^"\']+)["\']', script)
        if mset:
        ...
        patterns_matched.append("generate_random_email")
        consumed = True

    # ---- room_rate_plan_script
    elif ("roomRates" in script and "ratePlanCode" in script
          and "RoomTypeCode" in script):
        mstep = re.search(
            r"responseStepName\s*=\s*['\"]([^'\"]+)['\"]", script)
        mprops = re.search(
            r"propertiesStepName\s*=\s*['\"]([^'\"]+)['\"]", script)
        resp_name = mstep.group(1) if mstep else ""
        ns = mprops.group(1) if mprops else "Properties"
        resp_var = (response_var_by_step or {}).get(resp_name) or (
            re.sub(r"[^A-Za-z0-9_]", "_", resp_name) + "Res"
            if resp_name else "null")
        lines.append(
    ...
    # ---- cleanUp_difference: null out partitionId / endOffset
                    ...
                    script)
          and "endOffset" in script):
        tgt = "difference"
        mstep = re.search(
            r"getTestStepByName\(\s*['\"]([^'\"]+)['\"]", script)
        if mstep:
            tgt = mstep.group(1)
        lines.append('// [translated] cleanUp_difference -- clear partition cursor')
        lines.append(
            f'TestSupport.putExtracted(ctx, "{tgt}.partitionId", "");')
        lines.append(
            f'TestSupport.putExtracted(ctx, "{tgt}.endOffset", "");')
        patterns_matched.append("cleanup_difference")
        consumed = True
    ...
    if _context_expand_vars:
        ...
        for var, java_expr, ref in _context_expand_vars:
            ...
            # putExtracted: each Groovy script's `def X = context.expand`
            # is a fresh local. putIfNonEmpty (=putIfAbsent) pinned
            # hiltonmemberid to the owner TOTP id so member JDBC reused
            # the consumed OTP. Empty extracts still skip (putExtracted).
            lines.append(
    ...
    # ---- Selenium / HWS UI (LaunchHWSSearchLead, LaunchHWSSearchAccount)
    # ReadyAPI drives Chrome: login, open Lead, set Attestation Verified.
    # We do not emit Selenium. Config expands above still populate
    # leadID / url / credentials so later REST steps can run. The warn
    # must be loud -- expand-only used to look like the UI step succeeded.
    if re.search(
            r'org\.openqa\.selenium|ChromeDriver|webdriver\.chrome\.driver',
            script):
        hint = (step_name_hint or "LaunchHWS").replace("\\", "\\\\").replace('"', '\\"')
        lines.append(
            f'LOG.warn("STUB: ReadyAPI step \\"{hint}\\" is Selenium/HWS UI '
            f'and was NOT executed. Account stays limited unless attested '
            f'another way. TestNG group: needs-hws-ui");')
        patterns_matched.append("selenium_hws_ui_stub")
        consumed = True
    ...
    # ReadyAPI last-write-wins for this Groovy step. JSON extract and
    # location-header slice both write ctx via setPropertyValue; only
    # the last nonempty write per (step, field) is authoritative.
    last_setproperty = _last_nonempty_setproperty(script)
    ...
    if not consumed:
        _incomplete_guest = _emit_incomplete_guestid_extract(script, ctx)
        if _incomplete_guest:
            lines.extend(_incomplete_guest)
            patterns_matched.append("incomplete_guestid_extract")
            consumed = True
    ...
    if _loc_matches:
        ...
        if hm:
            ...
            _bare_var_rx = re.compile(
                r'^(\w+)(?:\.toString\(\))?(?:\.trim\(\))?$')
            loc_pubs: list[tuple[str, str, str]] = []
            for target_step, field, expr in _find_setproperty_targets(script):
                if expr in ('""', "''"):
                    continue
                if not _should_emit_setproperty(
                        last_setproperty, target_step, field, expr):
                    # Later setPropertyValue of this field wins
                    # (ReadyAPI last-write). Typical: split('/')[0]
                    # guest id then resp_obj.memberId.
                    continue
                use_idx = None
            ...
            if not loc_pubs:
                ...
                for use_idx, target_step, field in loc_pubs:
                    lines.append(
                        f'    if (parts_{hdr_id}.length > {use_idx}) '
    ...
    if (_EMAIL_ASSIGN_RX.search(script) or "generatedEmail" in script
            or ("def generator" in script and "setPropertyValue" in script)):
    ...
    if "space_concat_names" not in patterns_matched:
        overlay = _space_concat_overlay_lines(script, last_setproperty)
        if overlay:
            lines.append("{")
            lines.extend(overlay)
    ...
    for m in _NUMERIC_RANDOM_RX.finditer(script):
        ...
        # Stash on ctx so later JDBC (outside this scope block) can
        # still concat `${var}` without a compile error.
        lines.append(
            f'    TestSupport.putExtracted(ctx, "__groovy.{var}", {var});')
    if num_random_vars:
    ...
    # Numeric-random locals emitted above (`randomNumber`) are Java
    # identifiers in this method -- GString `${randomNumber}` must
    # concat them, not look for a ctx key.
    for _nr in num_random_vars:
        _local_java[_nr] = f'TestSupport.ctxGet(ctx, "__groovy.{_nr}")'
    ...
    # `def websiteDomains = "'"+ alias.getPropertyValue('Domain') +"'"` --
    # CleanupDB builds a single-quoted SQL literal from a Properties cell.
    for m in re.finditer(
            r'def\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*'
            r'["\']\'["\']\s*\+\s*'
            r'([A-Za-z_][A-Za-z0-9_]*)\.getPropertyValue\s*\(\s*'
        ...
        var, alias, field = m.group(1), m.group(2), m.group(3)
        step = _step_aliases.get(alias)
        if step and var not in _local_java:
            _local_java[var] = (
                f'"\'" + String.valueOf({_ctx_java(step, field)}) + "\'"')

    # context.expand('${...}') -- STEP#FIELD, STEP#Response#jsonPath,
    # #Project#, etc. Optional trailing `.replace("'", "''")` for SQL.
    _rv_map = response_var_by_step or {}
    for m in re.finditer(
            r'def\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*context\.expand\s*\(\s*'
            r'([\'"])(\$\{.+?\})\2\s*\)'
            r'(?:\s*\.\s*replace\s*\(\s*[\'"]\'[\'"]\s*,\s*[\'"]\'\'[\'"]\s*\))?',
            script):
        var, ref = m.group(1), m.group(3)
        if var in _local_java:
            continue
        java = _translate_soapui_ref_to_java_expr(ref, _rv_map)
        if '.replace("' in m.group(0):
            java = (f'({java} == null ? "" : String.valueOf({java}))'
                    f'.replace("\'", "\'\'")')
        _local_java[var] = java

    # CleanupDB / testdata-cleanup: quoted SQL IN-list from a Project CSV
    # property. ReadyAPI writes:
    #   def websiteDomains = context.expand('${#Project#WEBSITE_DOMAIN}')
    #       .split(',').collect { "'$it'" }.join(',')
    # Matching `.split` after the expand is enough -- collect/join quoting
    # is what Db.quotedCsvInList already does.
    for m in re.finditer(
            r'def\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*context\.expand\s*\(\s*'
            r'''['"]\$\{#Project#([A-Za-z0-9_.-]+)\}['"]\s*\)'''
            r'\s*\.split\b',
            script):
        var, proj_key = m.group(1), m.group(2)
        _local_java[var] = (
            f'Db.quotedCsvInList(Config.get("{proj_key}", ""))')

    # `def accountIdLong = accountID.toLong()` -- numeric cast of a
    # previously resolved Groovy local (usually a Properties step field).
    # Without this, triple-quoted SQL `${accountIdLong}` becomes a bare
    # Java identifier and javac fails.
    for m in re.finditer(
            r'def\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*'
            r'([A-Za-z_][A-Za-z0-9_]*)\s*\.\s*to(?:Long|Integer)\s*\(\s*\)',
            script):
        var, src = m.group(1), m.group(2)
        if var in _local_java:
            continue
        if src in _local_java:
            _local_java[var] = (
                f'Long.parseLong(String.valueOf({_local_java[src]}).trim())')
    ...
    def _compose_gstring_inner(inner: str) -> str | None:
        """Groovy GString body (no surrounding quotes) -> Java concat."""
        pos = 0
        subparts: list[str] = []
        for gm in re.finditer(r'\$\{([A-Za-z_][A-Za-z0-9_]*)\}', inner):
            lit = inner[pos:gm.start()]
            if lit:
                subparts.append(
                    ...
                    .replace("\r", "").replace("\n", "\\n") + '"')
            ref = gm.group(1)
            if ref in _local_java:
                subparts.append(_local_java[ref])
            else:
                # context.expand `def X = ...` already put X on ctx.
                # Do not emit a bare Java ident -- that fails compile
                # when X is scoped in another block.
                k = ref.replace("\\", "\\\\").replace('"', '\\"')
                subparts.append(f'TestSupport.ctxGet(ctx, "{k}")')
            pos = gm.end()
        tail = inner[pos:]
        if tail:
            subparts.append(
    ...
    # Groovy triple-quoted SQL, e.g. travel-agent whitelist:
    #   def sql_query = """
    #       INSERT INTO account_rules ... '${randomNumber}' ...
    #   """
    # A naive `def X = """` parse treated the opener as an empty string
    # plus a leftover quote and emitted `String __jdbcSql = "\"";`.
    for m in re.finditer(
            r'(?ms)^\s*def\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*"""(.*?)"""',
            script):
        var, inner = m.group(1), m.group(2)
        if var in _local_java:
            continue
        composed = _compose_gstring_inner(inner)
        if composed:
    ...
    for m in _DEF_ASSIGN_RX.finditer(script):
        var, rhs = m.group(1), m.group(2).strip().rstrip(';').strip()
        if var in _local_java:
            continue  # already resolved by an earlier pattern above
        # Triple-quoted strings are handled above; `"""` as RHS must
        # not be composed as `"\""`.
    ...
    for groups, args_body in _balanced_arg_call(
        ...
        if re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", query_expr) \
            ...
            lines.extend([
                ...
                f'        Db.executeComposed(__jdbcSql);',
        ...
        # Concatenation like `"DELETE ... IN (" + websiteDomains + ")"`
        # once every identifier is in `_local_java` (CleanupDB quoted
        # IN-list). Emit Db.execute instead of SkipException.
        if has_plus:
            composed_concat = _compose_java_from_concat(query_expr)
            if composed_concat is not None:
                preview_c = query_expr.replace("*/", "* /")[:80]
                lines.extend([
                    f'// [translated] JDBC execute (from Groovy concat '
                    f'`{preview_c}`)',
                    f'if (Db.isConfigured()) {{',
                    f'    try {{',
                    f'        String __jdbcSql = {composed_concat};',
                    f'        Db.executeComposed(__jdbcSql);',
                    f'    }} catch (Exception __jdbcEx) {{',
                    ...
                    f'    }}',
                    f'}} else {{',
                    f'    LOG.warn("Skipping JDBC step (Db not configured): concat SQL");',
                    f'}}',
                ])
                _mark("jdbc_execute_composed_concat")
                consumed = True
                continue
        if is_double_str or is_single_str:
            ...
            if is_select_query:
                ...
                lines.extend([
                    ...
                    f'        {result_var} = Db.queryComposed(__jdbcSql'
                    f'{params_java});',
                    f'    }} catch (Exception __jdbcEx) {{',
            ...
            preview_java = (query_expr[:80]
                            ...
                            .replace("\r", " ")
                            .replace("\n", " "))
            # B2B-5269 diff_address: `//def sql_addressline1 = ...` is
            # commented but `sql.execute(sql_addressline1)` stays live.
            # ReadyAPI's try/catch swallows MissingPropertyException --
            # warn-and-continue, not SkipException, when no active def.
            if (re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", query_expr)
                    and query_expr not in _local_java
                    and not _has_active_groovy_def(script, query_expr)):
                lines.extend([
                    f'// [jdbc] sql.execute(`{query_expr}`) -- Groovy local '
                    f'undefined (def commented out or missing); ReadyAPI '
                    f'catch swallows MissingPropertyException',
                    f'LOG.warn("Skipping JDBC execute: Groovy local '
                    f'`{query_expr}` was not defined");',
                ])
                _mark("jdbc_execute_undefined_local")
                consumed = True
                continue
    ...
    # composed_helper is the guard+log+run variant of java_helper: it performs
    # the unsafeSqlReasonForQuery check internally, so the emit site does not
    # repeat a 12-line ceremony per call.
    for method_name, java_helper, result_type, composed_helper in (
    ...
    for groups, args_body in _balanced_arg_call(
        ...
        # OTP extracts: one captured column -> Db.pollUntilStable
        # (4 attempts / 2s, 6-digit pad for email_otp). Multi-capture
        # OTP queries keep the inlined loop so sibling columns still
        # populate.
        _otp_simple = _is_otp_extract and len(captured_vars) == 1
        if _otp_simple:
            _otp_outer, _otp_field = captured_vars[0]
            lines.append(
                f'                {_otp_outer} = Db.pollUntilStable('
                f'__jdbcSql, "{_otp_field}");')
            lines.append(
                f'                LOG.info(" .. jdbc extract "'
                f' + "{_otp_outer}=<column:{_otp_field}>=" + '
                f'({_otp_outer} == null || {_otp_outer}.isEmpty() '
                f'? "<empty>" : {_otp_outer}));')
        elif _is_otp_extract and _otp_check_var:
            lines.append(
        ...
        if (not _otp_simple) and captured_vars and result_var:
            lines.append(
        ...
        if (not _otp_simple) and _is_otp_extract and _otp_check_var:
            _v = _otp_check_var
            lines.append(
