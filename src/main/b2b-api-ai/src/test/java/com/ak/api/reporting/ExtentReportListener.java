...
public class ExtentReportListener implements ITestListener, ISuiteListener, IConfigurationListener {
    ...
    public void onTestStart(ITestResult result) {
        ...
        String key = result.getTestClass().getRealClass().getName()
                ...
                + "@" + java.util.Arrays.deepHashCode(result.getParameters());
        if (key.equals(currentKey.get()) && currentTest.get() != null) {
            return;
        }
        currentKey.set(key);
    ...
    public void onTestFailure(ITestResult result) {
        ExtentTest t = currentTest.get();
        if (t == null) return;
        if (TestInvocations.supersededByRetry(result)) {
            t.warning("Retrying: " + safeMessage(result.getThrowable()));
            return;
        }
        renderExchanges(t);
    ...
    // ---------------------------------------------------------------------
    // Fallback: older BaseApiTest.assertAll() rethrew and TestNG fired
    // onConfigurationFailure instead of onTestFailure. Current BaseApiTest
    // stamps ITestResult.FAILURE and swallows, so onTestFailure is the
    // normal path. Keep this demote for any leftover throw.
    // ---------------------------------------------------------------------
    @Override
